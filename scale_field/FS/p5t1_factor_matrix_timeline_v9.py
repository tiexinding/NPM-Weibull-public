#!/usr/bin/env python3
"""Fig 7 v8 (260923): legend moved to the top of the figure and extended with the encodings formerly in the caption (block border, grey = different lengths, label rule); NaN cells (factors of different lengths) now actually drawn grey, as the caption said; colour-bar label no longer clipped. Colour scale unchanged (Blues 0-80%, 老丁 v7).
...
Writes 08_paper5_draft/p5_compile/figure_v2/figures/P5v2_S19_factor_topology_v7.png
"""
import json
...
FIG = ROOT / "08_paper5_draft/p5_compile/figure_v2/figures/P5v2_S19_factor_topology_v9.png"
...
gs = fig.add_gridspec(2, 2, wspace=0.08, hspace=0.16, left=0.085, right=0.80, top=0.875, bottom=0.04)
from matplotlib.colors import LinearSegmentedColormap as _LSC
def _trunc(name, a, b, n=256):
    c = plt.get_cmap(name); return _LSC.from_list(name + "_t", c(np.linspace(a, b, n)))
cmap = _trunc("Blues", 0.0, 0.80); cmap.set_bad("0.88")  # v7: sequential 0-1, capped at 80% depth (no pair below -0.10); cmap.set_bad("0.90")
im = None
for ax_i, t in enumerate(SHOW):
    ...
    assert np.nanmin(M) >= -0.10 - 1e-12, np.nanmin(M)   # legend states "no pair below -0.10"
    for e in edges:
    ...
    for i in range(N):
        for j in range(i + 1, N):
            ...
            if same_space[i, j] or abs(M[i, j]) >= LABEL_THRESH:
                ax.text(j, i, "1" if M[i, j] >= 0.995 else f"{M[i,j]:.2f}".lstrip("0").replace("-0", "-"), ha="center", va="center",
                        fontsize=8.6, color="white" if abs(M[i, j]) > .7 else "0.15", zorder=5)
    ...
    ax.set_xticklabels(LAB if r_ == 1 else [], rotation=90, fontsize=10)
    ax.set_yticklabels(LAB if c_ == 0 else [], fontsize=10)
    ...
    if c_ == 1:
        for gi, sp in enumerate(SPACES):
            ...
            ax.text(N - 0.2, (a + b - 1) / 2, SPACE_NAME[sp], fontsize=9.8, va="center", ha="left", color="0.2", linespacing=1.1)
cax = fig.add_axes([0.94, 0.22, 0.014, 0.40])
cb = fig.colorbar(im, cax=cax); cb.ax.tick_params(labelsize=10); cb.set_label(r"Spearman $\rho$ of the two factors (negatives, none below $-0.10$, shown as 0)", fontsize=10)
...
handles += [Patch(facecolor="white", edgecolor="k", lw=1.1, label="black border: one channel space (named at right)"),
            Patch(facecolor="0.88", edgecolor="none", label="grey: factors of different lengths"),
            Line2D([], [], color="none", label=f"numbers: within a space or $|\\rho| \\geq {LABEL_THRESH}$")]
fig.legend(handles=handles, loc="upper center", ncol=2, frameon=False, fontsize=9.8, bbox_to_anchor=(0.45, 1.0),
           handletextpad=0.5, columnspacing=1.6, labelspacing=0.35)
fig.savefig(FIG, dpi=220, facecolor="white", bbox_inches="tight")
