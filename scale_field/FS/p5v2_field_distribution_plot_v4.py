"""Paper#5 appendix figure v4 (260920): distinct arm palette + arm meanings in legends, (b) one colour per curve, all solid.
...
Writes p5_compile/figure_v2/figures/P5v2_S12_field_distribution_v4.png.
...
OUT = HERE.parents[1] / "figures" / "P5v2_S12_field_distribution_v4.png"
...
for i, f in enumerate(FAM):
    for ai, arm in enumerate(ARMS):
        ...
        for ax, key in ((ax1, "skew"), (ax2, "kurt_excess")):
            y = [m[key] for m in ms]
            ax.scatter(np.full(len(y), i + off), y, s=10, color=C_ARM[arm], alpha=0.6, lw=0, zorder=2)
...
ax1.set_ylabel("Skewness"); ax2.set_ylabel("Excess kurtosis")
ax1.tick_params(labelbottom=False)
ax2.set_xticks(xs); ax2.set_xticklabels([LAB[f] for f in FAM])
ax1.set_ylim(-4.5, 16.5)
ax1.set_title("(a)  Skewness and tail of the identity-axis field, grid at step 5,000", loc="left")
ax1.legend(handles=[Line2D([], [], marker="o", ls="", color=C_ARM[a], ms=5, label=ARM_LABEL[a]) for a in ARMS] +
...
ax.set_xlabel("Centred field  $h$"); ax.set_ylabel("Density")
ax.set_title("(b)  Example fields, seed 1, step 5,000", loc="left")
