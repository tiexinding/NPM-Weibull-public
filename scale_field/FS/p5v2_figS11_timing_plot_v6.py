...
OUT = ROOT / "08_paper5_draft/p5_compile/figure_v2/figures/P5v2_S11_timing_chain_v6.png"
...
plt.rcParams.update({"font.size": 9.5, "axes.titlesize": 10.5, "axes.labelsize": 10.5})
fig, axes = plt.subplots(3, 3, figsize=(11.0, 7.4), sharex=True, sharey="row", gridspec_kw=dict(wspace=0.10, hspace=0.18, top=0.83, bottom=0.08, left=0.07, right=0.985))
...
def line(ax, c, q, i, hollow=False):
    ...
    ax.plot(_sx(STEPS), traj[f"{i}|{c}|{q}"], ls="none", marker=MRK[c], ms=4.3, mfc="white" if hollow else col, mec=col, mew=1.0, zorder=4)
for j, (gname, comps) in enumerate(GROUPS):
    ...
    axes[2, j].set_xticks(_sx(xt)); axes[2, j].set_xticklabels(xl, fontsize=9)
    for r in range(3):
...
axes[2, -1].text(_sx(30000) + 0.04, K0, f"{K0:.3f}", fontsize=8, color="0.45", va="center", ha="left", clip_on=False)
...
l1 = fig.legend(handles=hk, loc="upper left", bbox_to_anchor=(0.07, 0.995), ncol=7, frameon=False, fontsize=9, handlelength=1.2, columnspacing=1.2, title="kind", title_fontsize=9)
l2 = fig.legend(handles=hx, loc="upper left", bbox_to_anchor=(0.07, 0.945), ncol=2, frameon=False, fontsize=9, handlelength=1.2, columnspacing=1.2, title="axis (middle row)", title_fontsize=9)
l3 = fig.legend(handles=hi_, loc="upper left", bbox_to_anchor=(0.40, 0.945), ncol=3, frameon=False, fontsize=9, handlelength=2.4, columnspacing=1.2, title="initialization", title_fontsize=9)
for l in (l1, l2, l3): l._legend_box.align = "left"
