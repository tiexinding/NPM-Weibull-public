"""Paper#5 Figure 2 v3 (260921): (b) fitting window drawn as two vertical dashed lines instead of a grey band (老丁). v2 (260916): a common normalized core, re-drawn for legibility.
...
OUT = HERE.parents[1] / "figures" / "P5v3_F1_normalized_core_v3.png"
...
gs = fig.add_gridspec(1, 5, width_ratios=[1.06, 0.24, 1.02, 0.46, 1.04], wspace=0.0,
...
ax.set_xlim(-0.55, len(SIZES) - 0.45); ax.set_ylim(1.03, 1.30)
ax.set_xlabel("Pythia model size"); ax.set_ylabel("Shape  $k$")
ax.set_title("(a)  Across public Pythia sizes", loc="left")
KLAB = {"q": "$q$", "k": "$k$", "v": "$v$", "o": "$o$", "ffn_in": "ffn in", "ffn_out": "ffn out"}
ha = [Line2D([], [], ls="none", marker=KMRK[k], color=KCOL[k], ms=5, label=KLAB[k]) for k in KINDS]
hs = [Line2D([], [], ls="none", marker="s", color="0.3", ms=5, label="raw"),
      Line2D([], [], ls="none", marker="s", mfc="white", mec="0.3", mew=1.3, ms=5, label="normalized"),
      Line2D([], [], ls="none", marker="*", mfc="white", mec="0.3", mew=1.1, ms=8, label="$v$, both axes")]
lga = ax.legend(handles=ha, loc="lower right", bbox_to_anchor=(0.72, 0.0), ncol=2, fontsize=7.8, frameon=False,
                handlelength=1.0, handletextpad=0.4, columnspacing=0.8, borderpad=0.2)
ax.add_artist(lga)
ax.legend(handles=hs, loc="lower right", fontsize=7.8, frameon=False,
...
ax.set_xlabel("Quantile $p$"); ax.set_ylabel("Core-profile deviation (%)")
...
lgb = ax.legend(handles=h1, fontsize=7.6, loc="upper left", bbox_to_anchor=(1.02, 1.0), frameon=False,
                title="family", title_fontsize=7.6, handlelength=1.3, handletextpad=0.4, borderpad=0.1)
ax.add_artist(lgb)
ax.legend(handles=h2, fontsize=7.6, loc="upper left", bbox_to_anchor=(1.02, 0.64), frameon=False,
          title="step", title_fontsize=7.6, handlelength=1.5, handletextpad=0.4, borderpad=0.1)
ax.text(np.mean(BAND), 0.975, "fitting window", transform=ax.get_xaxis_transform(),
        fontsize=7.4, color="0.45", ha="center", va="top")
...
ax.text(sx(30000) + 0.03, K0, f"{K0:.3f}", fontsize=7.6, color="0.45", va="center", ha="left")
...
ax.set_ylim(ylo - 0.02, yhi + 0.02)
ax.set_xlabel("Training step"); ax.set_ylabel(r"Core shape  $k_{\mathrm{bi}}$")
...
hc += [Line2D([], [], color="0.4", lw=2.2, marker="o", ms=4, mec="white", label=r"$k_{\mathrm{bi}}$"),
       Patch(facecolor="0.4", alpha=0.25, label="seed range"),
       Line2D([], [], color="0.4", lw=1.1, ls=(0, (3, 2)), label="raw $k$")]
ax.legend(handles=hc, loc="lower right", fontsize=7.8, frameon=False,
          handlelength=1.8, handletextpad=0.5, borderpad=0.2)
