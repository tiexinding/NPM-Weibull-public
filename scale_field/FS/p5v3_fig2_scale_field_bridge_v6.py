"""Paper#5 Figure 3 v5 (260923): legends at the top of every panel with raised y limits (house style); legend entries carry the caption explanations; include at 0.8\\textwidth so print font matches the 14.6-inch figures.
v3 (260916 late): common core + scale fields -> pooled shape through the mixture bridge.
...
OUT = HERE.parents[1] / "figures" / "P5v3_F2_scale_field_bridge_v6.png"
...
# ----------------------------- figure -----------------------------
plt.rcParams.update({"font.size": 11.5, "axes.titlesize": 12.5, "axes.labelsize": 12, "xtick.labelsize": 11, "ytick.labelsize": 11})
...
ax.text(xmax_a * 0.995, K_REF + 0.004, f"{K_REF:.3f}", fontsize=10.3, color="0.45", va="bottom", ha="right")
ax.set_ylim(float(kr_all.min()) - 0.03, float(kn_all.max()) + 0.10)
ax.set_yticks([0.90, 0.95, 1.00, 1.05, 1.10, 1.15, 1.20, 1.25])
...
h_state = [Line2D([], [], ls="none", marker="s", mfc="0.3", mec="0.3", ms=5, label=r"raw $k_{\mathrm{raw}}$ (filled)"),
           Line2D([], [], ls="none", marker="s", mfc="white", mec="0.3", mew=1.2, ms=5, label=r"row-normalized $k_{\mathrm{row}}$ (open), same matrix")]
lg = ax.legend(handles=h_kind, loc="upper left", bbox_to_anchor=(0.0, 1.0), ncol=5, fontsize=10.3, frameon=False,
               columnspacing=0.9, handletextpad=0.3, borderpad=0.2)
ax.add_artist(lg)
ax.legend(handles=h_state, loc="upper left", bbox_to_anchor=(0.0, 0.92), ncol=2, fontsize=10.3, frameon=False,
...
ax.plot(xs, ALPHA_WEIBULL * xs, color="0.55", lw=1.1, ls=(0, (5, 3)), marker="D", ms=4.2, mfc="white", mec="0.35", mew=0.9,
        markevery=(5, 12), zorder=6)   # drawn on top: coincides with the v/gate/up mean line
ax.set_xlim(0, h2max)
ax.set_ylim(-0.012, max(r["d"] for r in rows) * 1.12)
...
h_lines = [Line2D([], [], color=C_QK, lw=1.8, label=f"$q$/$k$ pooled fit, {qk['alpha']:.3f}"),
           Line2D([], [], color="0.25", lw=1.8, label=f"$v$/gate/up mean coefficient, {a_tra:.3f}"),
           Line2D([], [], color="0.55", lw=1.1, ls=(0, (5, 3)), marker="D", ms=4.2, mfc="white", mec="0.35", mew=0.9,
                  label=r"$6/\pi^2 = 0.608$, full-Weibull reference")]
lg = ax.legend(handles=h_pts, loc="upper left", bbox_to_anchor=(0.0, 1.0), ncol=1, fontsize=10.3, frameon=False,
               handletextpad=0.3, labelspacing=0.3, borderpad=0.2)
ax.add_artist(lg)
ax.legend(handles=h_lines, loc="upper left", bbox_to_anchor=(0.17, 1.0), ncol=1, fontsize=10.3, frameon=False,
...
h_c.append(Line2D([], [], color="0.45", lw=1.0, ls=(0, (5, 3)), label="identity $y = x$"))
ax.legend(handles=h_c, loc="upper left", ncol=4, fontsize=10.3, frameon=False, handlelength=1.2,
...
ax.set_xticks(xx); ax.set_xticklabels([LABEL[f] for f in FAM], fontsize=11)
for t, f in zip(ax.get_xticklabels(), FAM):
    t.set_color(COL[f])
ax.set_ylim(0, max(REL[f]["loao_max"] for f in FAM) * 1.40)
ax.set_ylabel(r"$|\widehat{k}_{\mathrm{raw}} - k_{\mathrm{raw}}|\,/\,k_{\mathrm{raw}}$, fold mean (%)")
ax.set_title("(d)  Held-out prediction error", loc="left")
h_d = [Patch(facecolor="0.75", label="in-sample"),
       Patch(facecolor="0.4", alpha=0.5, label="leave-one-run-out, median of 12 folds"),
       Patch(facecolor="0.4", label="leave-one-arm-out, median of 4 folds"),
       Line2D([], [], ls="none", marker="_", color="0.2", ms=10, mew=1.4, label="worst fold")]
ax.legend(handles=h_d, loc="upper left", ncol=2, fontsize=10.2, frameon=False, handlelength=1.4, handletextpad=0.5,
          labelspacing=0.3, columnspacing=1.0, borderpad=0.2)
