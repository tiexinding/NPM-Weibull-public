"""Paper#5 Figure 6 v4 (260923): caption encodings moved into the figure — column-side kinds marked "(col)" in the legend, step-axis convention in the x label, sample size printed in (a), line-style legend titled as read off (a), 6/pi^2 value and diamonds on the reference line (as Figs 3, 9), arrow legend states the direction.
v3 (260917): v2 + every panel carries its own legend in a headroom band at the top
...
OUT = HERE.parents[1] / "figures" / "P5v3_F4_field_evolution_v5.png"
...
def two_legends(ax, extras, extra_title=None, inline=False):
    if inline:   # extras share the kind legend's rows (saves one row of headroom)
        ax.legend(handles=kind_handles() + extras, loc="upper left", ncol=4, fontsize=8, frameon=False,
                  handletextpad=0.4, handlelength=2.2, columnspacing=0.8, labelspacing=0.3, borderpad=0.2,
    ...
    l1 = ax.legend(handles=kind_handles(), loc="upper left", ncol=4, fontsize=8, frameon=False,
                   ...
                   bbox_to_anchor=(0.0, 1.0))
    ax.add_artist(l1)
    l2 = ax.legend(handles=extras, loc="upper left", ncol=len(extras), fontsize=8, frameon=False, title=extra_title,
              title_fontsize=8, handletextpad=0.4, handlelength=2.4, columnspacing=1.2, borderpad=0.2,
              bbox_to_anchor=(0.0, 0.875))
    l2._legend_box.align = "left"
LBL = {"q": "$q$", "k": "$k$", "v": "$v$", "o": "$o$ (col)", "gate": "gate", "up": "up", "down": "down (col)"}
...
ax.set_xlabel("Training step  (linear to 800, log above)"); ax.set_ylabel(r"Field width  $H^W$")
ax.set_title("(a)  Field width over training", loc="left")
ax.grid(alpha=0.2, lw=0.5)
ax.set_ylim(0.02, 0.268); ax.set_yticks([0.05, 0.10, 0.15, 0.20, 0.25])
two_legends(ax, [Line2D([], [], ls="none", marker="v", mfc="0.6", mec="k", mew=0.5, ms=6, label="peak")], inline=True)
NMAT = {n for k in KINDS for n in J["rows"][k]["n"]}; assert len(NMAT) == 1, NMAT
ax.text(0.42, 0.02, f"{J['family'].capitalize()} initialization, median of {NMAT.pop()} matrices per point",
        transform=ax.transAxes, ha="center", va="bottom", fontsize=7.4, color="0.45")
...
ax.set_xticks(sx(XT)); ax.set_xticklabels(["0", "800", "3.2k", "10k", "30k"])
ax.set_xlim(sx(0) - 0.04, sx(30000) + 0.06)
ax.set_xlabel("Training step  (linear to 800, log above)"); ax.set_ylabel(r"$k_{\mathrm{raw}} - k_{\mathrm{ident}}$")
ax.set_title("(b)  Pooled-shape departure", loc="left")
ax.grid(alpha=0.2, lw=0.5)
ax.set_ylim(-0.0325, 0.0098); ax.set_yticks([0.0, -0.01, -0.02, -0.03])
two_legends(ax, [Line2D([], [], color="0.3", ls=LS[g], lw=1.3, label="$H^W$ " + GL[g]) for g in ("accumulate", "partial", "fall back")])

# ---- (c) bridge along the trajectory
ax = fig.add_subplot(gs[0, 4])
hmax = max(max(J["rows"][k]["H"]) for k in KINDS) ** 2 * 1.06
xs = np.linspace(0, hmax, 50)
ax.plot(xs, SIXPI * xs, color="0.55", lw=1.0, ls=(0, (5, 3)), marker="D", ms=3.6, mfc="white", mec="0.35", mew=0.8,
        markevery=(3, 9), zorder=5)
for k in KINDS:
    s = SH[k]; R = J["rows"][k]
    h2 = np.array(R["H"]) ** 2
...
ax.grid(alpha=0.2, lw=0.5)
ax.set_ylim(-0.002, 0.0425); ax.set_yticks([0.0, 0.01, 0.02, 0.03])
two_legends(ax, [Line2D([], [], color="0.55", lw=1.0, ls=(0, (5, 3)), marker="D", ms=3.6, mfc="white", mec="0.35", mew=0.8,
                        label=rf"slope $6/\pi^2 = {SIXPI:.3f}$"),
                 Line2D([], [], color="0.3", lw=1.2, marker=">", ms=5, ls="-", label="from step 0 to 30,000 (arrowhead)")])
