...
C_Q, C_K, C_V = "#B2182B", "#EF8A62", "#808080"; C_BASE, C_PERM, C_NOPE = "#404040", "#D95F02", "#D95F02"
...
NFP = len(prof["70m"]["q"])
plt.rcParams.update({"font.size": 9, "axes.titlesize": 10, "axes.labelsize": 9.5, "legend.fontsize": 7.8})
def clean(fig):
    for a in fig.axes: a.spines[["top", "right"]].set_visible(False); a.tick_params(labelsize=8.4)
...
for i, (key, lab) in enumerate((("ropeperm_by_coordinate", "indexed by matrix coordinate"), ("ropeperm_by_frequency", "re-indexed by assigned frequency"))):
    a = axc[i]; a.axhline(0, color="0.5", lw=.7)
    a.plot(x32, PA["pooled"]["q_proj"]["base"], color=C_BASE, lw=1.5); a.plot(x32, PA["pooled"]["k_proj"]["base"], color=C_BASE, lw=1.5, ls=(0, (4, 2)))
    a.plot(x32, PA["pooled"]["q_proj"][key], color=C_PERM, lw=1.5); a.plot(x32, PA["pooled"]["k_proj"][key], color=C_PERM, lw=1.5, ls=(0, (4, 2)))
    a.text(0.03, 0.97, lab, transform=a.transAxes, fontsize=8.2, color="0.3", va="top", ha="left"); a.set_xlabel("rotary pair index")
...
axc[1].set_yticklabels([]); axc[0].set_ylabel("row-scale profile  $h$")
axc[0].set_title("(a)  Profile follows frequency, not coordinate", loc="left")
axc[0].legend(handles=[Line2D([], [], color=C_BASE, lw=1.5, label="base run"), Line2D([], [], color=C_PERM, lw=1.5, label="permuted run")], frameon=False, loc="upper left", bbox_to_anchor=(0.0, 0.90), handlelength=1.4, borderaxespad=0.2)
axc[1].legend(handles=[Line2D([], [], color="0.3", lw=1.5, label="$q$"), Line2D([], [], color="0.3", lw=1.5, ls=(0, (4, 2)), label="$k$")], frameon=False, loc="upper left", bbox_to_anchor=(0.0, 0.90), handlelength=1.4, borderaxespad=0.2)
...
for si, s in enumerate(SEEDS):
    ...
    for kd in KS:
        ...
        ax.plot(xs, ys, "o", color={"q_proj": C_Q, "k_proj": C_K, "v_proj": C_V}[kd], ms=4, mec="white", mew=.5, zorder=4); ax.plot(xs, nl, "_", color="0.55", ms=6, mew=1.1, zorder=3)
    if si: ax.axvline(si * BW - 1.0, color="0.85", lw=.8)
    ax.text(si * BW + 2.5, -1.12, f"seed {s}", ha="center", va="center", fontsize=8.5, color="0.3")
ax.set_xticks([si * BW + L for si in range(3) for L in range(NL)]); ax.set_xticklabels([str(L) for si in range(3) for L in range(NL)]); ax.set_xlabel("layer")
ax.axhline(0, color="0.3", lw=.8); ax.set_ylim(-1.28, 1.45); ax.set_yticks([-0.5, 0, 0.5, 1.0]); ax.set_ylabel(r"frequency-over-coordinate advantage  $\Delta r_{\rm id}$")
ax.set_title("(b)  Replication across three paired seeds", loc="left")
ax.legend(handles=[Line2D([], [], ls="none", marker="o", color=c, ms=5, label=l) for c, l in ((C_Q, "$q$"), (C_K, "$k$"), (C_V, "$v$ control"))] + [Line2D([], [], ls="none", marker="_", color="0.55", ms=9, mew=1.3, label="layer-wise null 95%")],
          frameon=False, loc="upper left", ncol=2, columnspacing=1.0, handletextpad=.3)
# (c) Pythia
ax = fig.add_subplot(gs[0, 2]); shade = {"70m": 0.35, "160m": 0.65, "410m": 1.0}
for s in SIZES:
    for kd, ls, mk in (("q", "-", "o"), ("k", (0, (4, 2)), "s")):
        hc = prof[s][kd] - np.median(prof[s][kd]); ax.plot(range(NFP), hc / np.max(np.abs(hc)), color=C_Q if kd == "q" else C_K, alpha=shade[s], ls=ls, marker=mk, ms=3.5, lw=1.3, mfc="none")
ax.axhline(0, color="0.5", lw=.7); ax.set_xlabel("rotary pair index"); ax.set_ylabel("median-centred row-scale change  (/ max)"); ax.set_title("(c)  Transport across Pythia sizes", loc="left"); ax.set_ylim(-1.15, 1.75); ax.set_yticks([-1.0, -0.5, 0, 0.5, 1.0])
ax.legend(handles=[Line2D([], [], color=C_Q, alpha=shade[s], lw=1.5, label=s.upper()) for s in SIZES] + [Line2D([], [], color="0.3", marker="o", mfc="none", ms=3.5, lw=1.3, label="$q$"), Line2D([], [], color="0.3", ls=(0, (4, 2)), marker="s", mfc="none", ms=3.5, lw=1.3, label="$k$")], frameon=False, loc="upper left", ncol=2, columnspacing=1.0)
for a_ in (axc[0], axc[1], ax): a_.annotate("high freq.", xy=(0.0, -0.175), xycoords="axes fraction", fontsize=7.2, color="0.45", ha="left"); a_.annotate("low freq.", xy=(1.0, -0.175), xycoords="axes fraction", fontsize=7.2, color="0.45", ha="right")
clean(fig); fig.savefig(FIG / "P5v3_F5_rope_v5.png", dpi=220, bbox_inches="tight", facecolor="white"); print("wrote P5v3_F5_rope_v5.png")

# ================= Fig B readout =================
fig, axs = plt.subplots(1, 2, figsize=(10.5, 3.7), gridspec_kw={"wspace": 0.32, "width_ratios": [1.3, 1.0]})
ax = axs[0]; vals = []; xt = []; xl = []
for si, s in enumerate(SEEDS):
    j3 = MS["seeds"][str(s)]["reduced"]["J3"]
    for ki, kd in enumerate(KS):
        ...
        xt.append(x); xl.append(LAB[kd])
    ax.text(si * 3.6 + 1, 1.12, f"seed {s}", ha="center", fontsize=9, color="0.3")
...
ax.legend(handles=[Patch(facecolor="0.45", label=r"$r_{\rm freq}$, by frequency (kind colour)"), Patch(facecolor="0.80", label=r"$r_{\rm coord}$, by coordinate")], frameon=False, loc="upper left", ncol=1, handlelength=1.0, handletextpad=0.4, labelspacing=0.3, borderpad=0.1, fontsize=8.5, bbox_to_anchor=(0.0, 1.0))
...
axk.bar(idx - w / 2, dk, w, color=A_BASE, label="pooled $k$"); axk.bar(idx + w / 2, dkr, w, color=A_LIGHT, label="$k_{\\rm row}$")
...
axk.set_title("(b)  Pooled shape invariant, width not  (seed 1, illustrative)", loc="left")
...
clean(fig); fig.savefig(FIG / "P5v3_SB_rope_readout_v3.png", dpi=220, bbox_inches="tight", facecolor="white"); print("wrote P5v3_SB_rope_readout_v3.png")

# ================= Fig B NoPE =================
fig, axs = plt.subplots(1, 4, figsize=(15.0, 3.7), gridspec_kw={"wspace": 0.38, "width_ratios": [1.15, 1.25, 0.7, 1.0]})
...
ax.set_xticks(_sx([0, 800, 3200, 10000, 30000])); ax.set_xticklabels(["0", "800", "3.2k", "10k", "30k"]); ax.set_xlabel("training step"); ax.set_ylabel("$H^W_{\\rm row}$ of $q$, $k$"); ax.set_title("(a)  A $q/k$ row field forms without RoPE", loc="left")
...
ax.axhline(0, color="0.5", lw=.7); ax.set_xlabel("rotary pair index  (coordinate label only for NoPE)"); ax.set_ylabel("row-scale profile at 30k"); ax.set_title("(b)  Rotary-pair-indexed profile at 30k", loc="left")
...
ax.set_xticks([0]); ax.set_xticklabels(["$q/k$ by RoPE pair"]); ax.set_xlim(-0.7, 0.7); ax.set_ylim(0, 1.05); ax.set_ylabel("$q$--$k$ profile correlation at 30k"); ax.set_title("(c)  Pair coupling", loc="left")
...
ax.legend(handles=[Patch(facecolor=A_BASE, label="RoPE (base)"), Patch(facecolor=A_ALT, label="NoPE"), Line2D([], [], ls="none", marker="_", color="0.45", ms=10, mew=1.4, label="re-pairing null")], frameon=False, loc="upper left", ncol=2, columnspacing=0.8)
clean(fig); fig.savefig(FIG / "P5v3_SB_nope_v4.png", dpi=220, bbox_inches="tight", facecolor="white"); print("wrote P5v3_SB_nope_v4.png")
