"""Fig 8 v3 (260923): caption encodings moved into the panels (evaluation-slice legend wording, unit line labelled in (b)).
...
OUT = HERE.parents[1] / "figures" / "P5v3_F8_gain_edit_v4.png"
...
fig, (ax, bx, cx) = plt.subplots(1, 3, figsize=(14.2, 5.4), gridspec_kw=dict(width_ratios=[0.8, 1.35, 1.25], wspace=0.42,
...
ax.text(0, 0.012, r"$|\Delta L|\leq %.1f\times10^{-7}$" % (np.ceil(max(BAL.values()) * 1e8) / 10) + "\non all paths,\nboth slices", ha="center", va="bottom", fontsize=10.5, color="0.25")
...
la = ax.legend(handles=ha, loc="upper left", ncol=1, frameon=False, fontsize=10.5, handlelength=1.0, bbox_to_anchor=(0.0, 1.0)); ax.add_artist(la)
hb = [Line2D([], [], ls="none", marker="s", color="0.3", ms=7, label="eval slice 1"),
      Line2D([], [], ls="none", marker="s", mfc="white", mec="0.3", ms=7, mew=1.2, label="eval slice 2")]
ax.legend(handles=hb, loc="upper right", ncol=1, frameon=False, fontsize=10.5, handlelength=1.0, bbox_to_anchor=(1.0, 1.0))
ax.set_ylabel(r"$\Delta L$ (nats / token)")
ax.set_title("(a)  Balance and gain", loc="left")
ax.grid(axis="y", alpha=0.2, lw=0.5)

# (b) perturbation-matched edits, cost relative to flatten
CATS = [("flatten1", "flatten\n$-g$"), ("shuffle_matched", "permuted\n$\\frac{g_\\pi-g}{\\sqrt{2}}$"), ("gauss", "Gauss.\n$-\\xi$"),
        ("signflip", "rand.\nsigns\n$-\\sigma g$"), ("amplify", "log-gain\ndoubled")]
...
bx.text(1.5, 1.03, "1 = flatten, per unit displacement", ha="center", va="bottom", fontsize=10, color="0.4")
bx.set_xticks(range(len(CATS))); bx.set_xticklabels([c[1] for c in CATS])
bx.set_ylabel(r"$(\Delta L_{\rm edit}/\Delta L_{\rm flat})\,/\,(\Delta_F^{\rm edit}/\Delta_F^{\rm flat})$")
...
l1 = bx.legend(handles=h1, loc="upper left", ncol=3, frameon=False, fontsize=10.5, handlelength=1.2, columnspacing=1.0, bbox_to_anchor=(0.0, 1.0)); bx.add_artist(l1)
h2 = [Line2D([], [], ls="none", marker="o", color="0.3", ms=6, label="eval slice 1"),
      Line2D([], [], ls="none", marker="o", mfc="white", mec="0.3", ms=6, mew=1.3, label="eval slice 2"),
      Line2D([], [], color="0.3", lw=1.0, label="min–max over 5 random draws")]
bx.legend(handles=h2, loc="upper left", ncol=3, frameon=False, fontsize=10.5, handlelength=1.2, columnspacing=1.0, bbox_to_anchor=(0.0, 0.92))
...
for p in PATHS:
    for s, E in (("slice1", E1), ("slice2", E2)):
        ...
        for d in range(10):
            if y[d] <= 0:  # non-positive response: drawn at the floor as a hollow downward triangle
                NEG.append((p, s, d + 1, float(y[d])))
                cx.plot(xs[d], FLOOR, "v", mfc="white", mec=COL[p], ms=7, mew=1.4, zorder=5)
            else:
...
lc = cx.legend(handles=hc, loc="upper left", ncol=3, frameon=False, fontsize=10.5, handlelength=1.6, columnspacing=1.0, bbox_to_anchor=(0.0, 1.0)); cx.add_artist(lc)
hd = [Line2D([], [], color="0.3", lw=1.2, marker="o", ms=5, label="eval slice 1 (solid, filled)"),
      Line2D([], [], color="0.3", lw=1.2, ls="--", marker="o", mfc="white", mec="0.3", ms=5, mew=1.3, label="eval slice 2 (dashed, hollow)"),
      Line2D([], [], ls="none", marker="v", mfc="white", mec="0.3", ms=7, mew=1.4, label=r"$\Delta L \leq 0$, drawn at floor")]
cx.legend(handles=hd, loc="upper left", ncol=2, frameon=False, fontsize=10.5, handlelength=1.8, columnspacing=1.0, bbox_to_anchor=(0.0, 0.92))
cx.set_yticks([1e-3, 1e-2, 1e-1, 1]); cx.set_yticklabels([r"$\leq 10^{-3}$", "0.01", "0.1", "1"])
cx.set_xticks(xs); cx.set_xlabel("decile of $g$  (1 = most suppressed, 10 = most amplified)", fontsize=11.5, ha="right", x=1.0)
cx.set_ylabel(r"$\Delta L_{\rm decile}\,/\,\Delta L_{\rm full\ flatten}$  (log)")
cx.set_title("(c)  Sensitivity across gain deciles", loc="left")
cx.grid(alpha=0.2, lw=0.5, which="major")
for a in fig.axes:
    a.spines[["top", "right"]].set_visible(False)
assert len(NEG) == 7, NEG; print('non-positive decile responses:', NEG)
