"""Paper#5 appendix figure v2 (260919): panels (a)(b) transposed to the Figure 2 convention
...
SM = DRAFT.parent
OUT = HERE.parents[1] / "figures" / "P5v3_SB_bridge_coefficients_v2.png"
...
fits = D["fits"]
MODELS = (("M1", "0.62", r"$H^W_{\mathrm{row}}$ only"), ("M2", C_VO, r"$+\,H^W_{\mathrm{col}}$"), ("M3", C_QK, r"$+$ cross term"))
for f in FAM7:
...
for i, f in enumerate(FAM5):
    ...
    ax.text(i, hi + 0.02, rf"$R^2_0$={p['R0_sq']:.3f}", ha="center", va="bottom", fontsize=7.6, color="0.35")
ax.axhline(ALPHA_WEIBULL, color="0.6", lw=0.9, ls=(0, (4, 3)), zorder=1)
ax.text(len(FAM5) - 0.55, ALPHA_WEIBULL, r"$6/\pi^2$", ha="right", va="bottom", fontsize=8, color="0.45")
ax.set_xticks(range(len(FAM5))); ax.set_xticklabels([LAB[f] for f in FAM5])
ax.set_xlim(-0.6, len(FAM5) - 0.4)
ax.set_ylim(0.36, max(A["partA"][f]["run_alpha_range"][1] for f in FAM5) + 0.16)
ax.set_ylabel(r"Protocol coefficient $\alpha_P$")
ax.set_title(r"(a)  $\alpha_P$ per kind, pooled and per run", loc="left")
h_a = [Line2D([], [], ls="none", marker="o", mfc="white", mec="0.3", mew=1.5, ms=6, label="pooled fit"),
       Line2D([], [], color="0.3", lw=2.6, alpha=0.25, label="range over 12 runs")]
ax.legend(handles=h_a, loc="lower right", fontsize=8.2, frameon=False, handlelength=1.6, borderpad=0.2)
...
for j, (m, c, lab) in enumerate(MODELS):
    ...
    ax.bar(x + (j - 1) * w, med, w, color=c, alpha=0.9, zorder=3)
...
ax.set_ylabel(r"Held-out MAE of predicted $k_{\mathrm{raw}}$")
ax.set_title("(c)  Held-out error by model", loc="left")
h_c = [Line2D([], [], marker="s", ls="none", color=c, ms=8, label=lab) for _, c, lab in MODELS]
h_c.append(Line2D([], [], marker="_", ls="none", color="0.15", ms=8, mew=1.2, label="worst fold"))
ax.legend(handles=h_c, loc="upper left", fontsize=8, frameon=False, handlelength=1.0, handletextpad=0.5, borderpad=0.2)
