...
Per block and kind (q, k, v split from the fused per-head QKV as in phase1b; o; ffn_in; ffn_out):
  ...
  k_both     after tracked two-sided balancing (v2; identical to the self-trained pipeline)
...
OUT = ROOT / "08_paper5_draft/data/p5_public_pythia_knorm_v2.json"
...
def bothnorm(w, it=200, tol=1e-6):
    """Tracked two-sided balancing, identical to bothnorm_tracked of p5v2_ea_kbi_trajectory_v1.py
    (all-ones start, rows before columns, final global rescale split equally, 1e-9 reconstruction check)."""
...
json.dump({"schema": "p5-public-pythia-knorm-v2", "protocol": "weibull_fit_1024 middle-80%; identity-axis normalization rows q/k/v/ffn_in, cols o/ffn_out; both = tracked alternating balancing (tol 1e-6, 200 sweeps, 1e-9 reconstruction check; v2 replaces the v1 routine of 60 sweeps / tol 1e-5)",
