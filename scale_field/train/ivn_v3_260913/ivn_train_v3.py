...
# --- 260913 机理队列新增 ---
ap.add_argument("--v_window", default=None,
                help="C2: START,END (含端点); 只在窗内施加 --arm vmod, 窗外走标准 Adam。不给=全程(与既有行为一致)")
ap.add_argument("--resume_at", default="",
                help="C3: 逗号分隔步号; 在这些步另存具名快照 resume_step{t}.pt (常规 resume.pt 仍只留最近一份)")
ap.add_argument("--rebranch_seed", type=int, default=None,
                help="C4: resume 之后重设取批 RNG; 其余状态(权重/优化器/累加器)不变 ⇒ 只改后续数据顺序")
a = ap.parse_args()
...
V_WIN = None
if a.v_window is not None:
    assert ARM == "vmod", "--v_window only with --arm vmod"
    _vw = [int(x) for x in a.v_window.split(",")]
    assert len(_vw) == 2 and 1 <= _vw[0] <= _vw[1], f"--v_window 非法: {a.v_window}"
    V_WIN = tuple(_vw)
RESUME_AT = sorted({int(x) for x in a.resume_at.split(",") if x.strip()})
ARM_TAG = ARM if ARM not in ("vmod", "mmod") else (f"vmod:{a.vmode}:rn={a.rn}:win={a.v_window}" if ARM == "vmod" else f"mmod:{a.mmode}:{a.m_window}:rn={a.rn}")
...
def save_resume(t, loss200):
    ...
    if t in RESUME_AT:                     # C3: 另存一份具名快照供分支使用
        import shutil
        shutil.copyfile(f"{a.out}/resume.pt", f"{a.out}/resume_step{t}.pt")
        print(f"[IVN-{ARM_TAG}] C3: 快照 resume_step{t}.pt 已写", flush=True)
...
if a.resume:
    ...
    if a.rebranch_seed is not None:        # C4: 只改后续数据顺序，其余状态原样
        assert a.data_arm is None, "--rebranch_seed 仅用于无 data_arm 的随机取批"
        rng = np.random.default_rng(a.rebranch_seed)
        print(f"[IVN-{ARM_TAG}] C4: 取批 RNG 已重设为 seed={a.rebranch_seed} "
              f"(权重/优化器/累加器保持 resume 状态)", flush=True)
    START = sd["step"] + 1
...
for t in range(START, T + 1):
    ...
    if ARM in ("vmod", "mmod"):
        ...
        v_active = (ARM == "vmod") and (V_WIN is None or V_WIN[0] <= t <= V_WIN[1])
        need_redo = v_active or in_win or mreset_local
        if need_redo:
            with torch.no_grad():
                for nm in ANA:
                    ...
                    if ARM == "vmod":
                        # C2: 窗外恢复标准 Adam (v_active 为假时本分支不会进入 need_redo)
